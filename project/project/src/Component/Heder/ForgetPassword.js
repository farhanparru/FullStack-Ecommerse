...
const ForgetPassword = () => {
  ...
  const userValid = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/users/forgotpassword/${id}/${token}`, {
        headers: {
  ...
  const sendPassword = async (e) => {
    ...
    try {
      const response = await axios.post(`http://localhost:3000/api/users/${id}/${token}`, {
        password: password // Correct payload format
      }, {
        headers: {
          'Content-Type': 'application/json'
        }
            ...
            <form className="mt-5" onSubmit={sendPassword}>
              ...
              <div className="grid gap-y-4">
                ...
                <div>
                  ...
                  <p className="hidden mt-2 text-xs text-red-600" id="confirm-new-password-error">Please include a password that complies with the rules to ensure security</p>
                </div>
                <button
                  type="submit"
